 ...
 * Author: Oceane Bel
 *
 * Slow-DDoS variant of ns3-helics-grid-dnp3.cc: a separate scenario file,
 * not a modification of the original. The original stays UDP-only
 * (EnableTCP=false) since replay/FDI/MIM were already validated against
 * it there, and handle_MIM's DNP3 byte-offset parsing assumes UDP's
 * datagram framing -- switching that file's transport to TCP would put
 * those results at risk of needing re-validation for a reason unrelated
 * to slow-DDoS. This copy exists solely to give the SlowlorisBotApplication
 * connection-exhaustion bot a real DNP3 TCP listening socket to attack
 * (EnableTCP=true below), scoped to this file only.
 *
...
main (int argc, char *argv[])
{
  ...
  for (i = 0;i < configObject["microgrid"].size();i++)
  {
    ...
    // points files for MMS's 123-bus validation; this original DNP3 loop
    // is where every protocol's copy of it ultimately came from. Using
    // `i` directly matches what's actually on disk for the current config.
  ...
  if (includeMIM == 1){
    for (int x = 0; x < val.size(); x++){ //std::stoi(configObject["MIM"][0]["NumberAttackers"].asString()); x++){
      ...
      dnp3MIM1.SetAttribute("EnableTCP", BooleanValue (true));
